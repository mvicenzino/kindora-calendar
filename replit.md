...
- **Resources Section (Documents)**: The Documents page has two tabs — "Document Vault" (existing care documents) and "Resources" (interactive guides and checklists). First resource: an interactive "Parenting Aging Parents Checklist" with 4 sections (Personal, Health, Finances, Legal) and 22 items. Checkbox state persists in localStorage per family. Progress bar on card + within checklist. Section-level "Check all / Uncheck all" toggle. "Coming soon" placeholder card for future guides. Frontend only — no backend/DB needed. Component: `client/src/components/ResourcesSection.tsx`.
