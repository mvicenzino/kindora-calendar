...
### Integrations
- **Email Services**: Unified `sendEmail()` helper in `server/emailService.ts` — tries Resend first (`RESEND_API_KEY`), falls back to SendGrid (`SENDGRID_API_KEY`). `EMAIL_FROM_ADDRESS` sets the from address (required for Resend — must be a verified domain). Used for: weekly summaries, emergency bridge, family invites, support contact form, beta feedback notifications, and **automated welcome emails** (sent on first sign-up via all three auth paths: local, Replit OAuth, Google OAuth — skipped for demo users). Welcome email is from "Mike at Kindora" with `reply_to: mike@kindora.ai`. `sendWelcomeEmail()` is fire-and-forget (non-blocking, errors logged but don't break registration).
